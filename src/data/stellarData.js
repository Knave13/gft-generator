var radiuskm = [
  29.9, 59.8, 104.7, 149.6, 239.3, 418.9, 777.9, 1495.9, 2932.0, 5804.0, 11548.0, 23038.0, 46016.0, 91972.0, 183885.0, 367711.0, 735363.0, 1470666.0, 2941274.0, 5882488.0, 99, 99, 99 ]
var radiusau = [
  .2, .4, .7, 1.0, 1.6, 2.8, 5.2, 10.0, 19.6, 38.8, 77.2, 154.0, 307.6, 614.8, 1229.2, 2458.0, 4915.5, 9830.7, 19660.9, 39321.4, 78643.0, 157286, 314572 ]
var cloudiness = {"None": 0, "0": 0, "10": 0, "20": 10, "30": 10, "40": 20, "50": 30, "60": 40, "70": 50, "80": 60, "90": 70, "100": 70 } 
var greenhouse = [0, 0, 0, 0, 5, 5, 10, 10, 15, 15, 10, 0, 0, 15, 10, 0]
var albedo = {"Veldt": 0.1, "Desert": 0.2, "Water": 0.02, "Ice": 0.85, "Snow": 0.85, "DirtyIce": 0.55, "Mountain": 0.15, "Clouds": 0.4}
...
var latinSuffixes = [
  "", "Primus", "Secundus", "Tertius", "Quartus", "Quintus", "Sextus", "Septimus", "Octavus", "Nonus"]
var numericSuffixes = [
  "", "1", "2", "3", "4", "5", "6", "7", "8", "9"]
var romanSuffixes = [
  "", "I", "II", "III", "IV", "V", "VI", "VII", "VIII", "IX"]
var systemNature = [
  "Empty", "Solitary", "Binary", "Trinary", "Quadinary", "Quintinary", "Sextinary", "Septinary", "Octinary", "Noninary"]
var atmosphereNames = [
  "None", "Trace", "Very thin, tainted", "Very thin", "Thin, tainted", "Thin", "Standard", "Standard, tainted", "Dense", "Dense, tainted", "Exotic", "Corrosive", "Insidious", "Dense, high", "Ellipsoid", "Thin, low"]
...
var StarOrbitType = Object.freeze({
  "Primary": "Primary",
  "Close": "Close",
  "Near": "Near",
  "Far": "Far",
...
var stellarData = {
  ...
  stellarDataByKey: (starKey, callback) => {
    getStellarDataByKey(starKey, (data) => {
      callback(data)
    })
  },
  stellarData: (typeCode, classification, sizeCode, callback) => {
    var key = typeCode + classification + sizeCode
    getStellarDataByKey(key, (data) => {
      callback(data)
    })
  },
...
function getStellarDataByKey (key, callback) {
  dbConnection.collection('astronomics').doc(key)
  .get()
  .then(doc => {
      if (doc.exists) {
          callback(doc.data())
